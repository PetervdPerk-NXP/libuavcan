...
#include <uavcan_zephyr/clock.hpp>
#include <uavcan_zephyr/exception.hpp>
...
class SocketCanIface : public uavcan::ICanIface
{
    ...
    int read(uavcan::CanFrame& frame, uavcan::UtcTime& ts_utc, bool& loopback) const
    {
        ...
        const ssize_t nbytes = zsock_recvfrom(fd_, &receive_frame, sizeof(struct can_frame),
        ...
        ts_utc = uavcan::UtcTime::fromUSec(SYS_CLOCK_HW_CYCLES_TO_NS(k_cycle_get_32())/1000); //Cheasy I know
    ...
    void pollWrite()
    {
        while (hasReadyTx())
        {
            ...
            if (tx.deadline >= clock_.getMonotonic())
            {
                ...
                if (res == 1)                   // Transmitted successfully
                {
                    ...
                    {
                    	//std::cout << "Pending loopback " << tx.frame.id << std::endl;
        ...
        //std::cout << "TX queue full" << frames_in_socket_tx_queue_ << std::endl;
    }
    ...
    void pollRead()
    {
        while (true)
        {
            ...
            //std::cout << "RX " << " Time: " << clock_.getMonotonic().toUSec() << " FrameTime: " << rx.ts_utc.toUSec() << std::endl;
            ...
            if (res == 1)
            {
                ...
                if (accept)
                {
                	//std::cout << "Accept push rx " << rx.ts_utc << std::endl;
...
public:
    ...
    static int openSocket(const std::string& iface_name)
    {
        ...
        const int s = zsock_socket(PF_CAN, SOCK_RAW, CAN_RAW);
        if (s < 0)
        {
            return s;
        }
        ...
        // Bind to the specified CAN iface
        {
            ...
            addr.can_ifindex = net_if_get_by_iface(
            		net_if_get_first_by_type(&NET_L2_GET_NAME(CANBUS_RAW)));
            if (zsock_bind(s, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0)
            {
                return -1;
            }
        }
        ...
        // Configure
        {
            ...
            /*if (zsock_setsockopt(s, SOL_SOCKET, SO_TIMESTAMP, &on, sizeof(on)) < 0)
            {
                return -1;
            }*/
            // Socket loopback UNSUPPORTED in Zephyr SocketCAN
            /*if (zsock_setsockopt(s, SOL_CAN_RAW, CAN_RAW_RECV_OWN_MSGS, &on, sizeof(on)) < 0)
            {
                return -1;
            }*/
            // CAN filter
            if (zsock_setsockopt(s, SOL_CAN_RAW, CAN_RAW_FILTER, &filter, sizeof(filter)) < 0)
            {
            	return -1;
            }
        ...
        // Validate the resulting socket
        {
            ...
            (void)zsock_getsockopt(s, SOL_SOCKET, SO_ERROR, reinterpret_cast<void*>(&socket_error), &errlen);
            if (socket_error != 0)
            {
                errno = socket_error;
                return -1;
...
class SocketCanDriver : public uavcan::ICanDriver
{
    ...
    public:
        ...
        void updateDownStatusFromPollResult(const zsock_pollfd& pfd)
        {
            assert(pfd.fd == this->getFileDescriptor());
            if (!down_ && (pfd.revents & ZSOCK_POLLERR))
            {
                int error = 0;
                ::socklen_t errlen = sizeof(error);
                (void)zsock_getsockopt(pfd.fd, SOL_SOCKET, SO_ERROR, reinterpret_cast<void*>(&error), &errlen);
...
public:
                        ...
                        uavcan::MonotonicTime blocking_deadline) override
    {
        ...
        {
            // Poll FD set setup
            zsock_pollfd pollfds[uavcan::MaxCanIfaces] = {};
            ...
            for (unsigned i = 0; i < ifaces_.size(); i++)
            {
                if (!ifaces_[i]->isDown())
                {
                    ...
                    pollfds[num_pollfds].events = ZSOCK_POLLIN;
                    if (ifaces_[i]->hasReadyTx() || (inout_masks.write & (1U << i)))
                    {
                        pollfds[num_pollfds].events |= ZSOCK_POLLOUT;
                    }
            ...
            // Blocking here
            const int res = zsock_poll(pollfds, num_pollfds, timeout_usec/1000);
            if (res < 0)
            {
                return res;
            }
            ...
            for (unsigned i = 0; i < num_pollfds; i++)
            {
                ...
                const bool poll_read  = pollfds[i].revents & ZSOCK_POLLIN;
                const bool poll_write = pollfds[i].revents & ZSOCK_POLLOUT;
     ...
     * @throws uavcan_zephyr::Exception.
     */
